...
### Read Data
```{r}
dataset = read.csv(file="POS.csv",head=TRUE)
...
width=600, height=350)
ggboxplot(
  dataset, x = "gender", y = colnames(dataset)[11:18], ylab = "word count",
  merge = TRUE, palette = "jco"
  )
```

## Remove low mean variable
```{r}
dataset = subset(dataset, select=-c(X,Symbol,Interjection))
```
...
```{r}
dataset = subset(dataset, select=-c(Space))
```
...
manova_model= Manova(model, test.statistic = "Pillai")
```
...
welch = dataset %>% 
  gather(key = "variable", value = "value", -gender) %>%
  group_by(variable) %>%
  rstatix::welch_anova_test(value ~ gender)%>%
  select(-c(.y.,n,method))
...
model = lm(dvar ~ gender, dataset.outlierf)
